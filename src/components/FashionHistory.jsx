...
const FashionHistory = ( {data} ) => {
    return(
        <div className="cntre ptop">
            <div className="headline txt-upper">{data.headline}</div>
                            ...
                            return(
                                <div className="grid-item" key={i}>
                                    ...
                                    <div className="grid-gap ptop20">
                                        <div>
                                            <a className="btn btnb" href="#">{data.btn1Txt}</a>
                                            <a className="btn btnb" href="#">{data.btn2Txt}</a>
                                        </div>
                                        <div className="title-20 ptop20">{el.title}</div>
                                        <div className="crt-by">
                                            <img src={data.hIcon} alt="icon" /> {data.createdBy}
                                            <img src={data.bIcon} alt="icon" /> {data.time.month} {data.time.day} {data.time.year}
                                        </div>
                ...
                <div className="categories grid-gap res-m-ato">
                    <div className="cat-title cntre2 txt-upper">{data.catTitle}</div>
                    {
                        data.categories.map( (el, i) => {
                            return(
                                <div className="cat-item txt-ft-16-400 hov">
                                    <div>{el.category} </div> 
